...
export default function TaxPage() {
  ...
  // ── Monthly tax from dedicated endpoint (mirrors mobile getMonthlyTax) ────
  const { data: monthlyTaxData } = useQuery({
    queryKey: ['monthly-tax', selectedMonth + 1, selectedYear],
    queryFn: () => getMonthlyTax(String(selectedMonth + 1), String(selectedYear)),
    retry: false,
  });

  // ── Quarterly tax from dedicated endpoint (mirrors mobile getQuarterlyTax) ─
  const { data: quarterlyTaxData, isLoading: ql } = useQuery({
    queryKey: ['quarterly-tax', selectedYear, selectedQuarter],
    queryFn: () => getQuarterlyTax(String(selectedYear), String(selectedQuarter)),
  ...
  // Prefer backend monthly tax data if available (mirrors mobile)
  const monthlyPayload: any = (monthlyTaxData?.data as any) || (monthlyTaxData as any) || {};
  const estimatedTaxPool = monthlyPayload?.estimatedTaxPool ?? monthlyPayload?.taxPool ?? estimatedTax;

  // Quarterly data
  const qPayload: any = (quarterlyTaxData?.data as any) || (quarterlyTaxData as any) || {};
  const qRevenue  = qPayload?.totalRevenue  ?? 0;
  const qExpenses = qPayload?.totalExpenses ?? 0;
  const qVatIn    = qPayload?.vatCollected  ?? qPayload?.vatIn  ?? 0;
  const qVatOut   = qPayload?.vatPaid       ?? qPayload?.vatOut ?? 0;
  const qNetVat   = qVatIn - qVatOut;
  const qProfit   = qRevenue - qExpenses;
  const qEstTax   = qProfit > 0 ? qProfit * 0.30 : 0;
  ...
  const metricCards = [
    ...
    { label: 'Estimated Tax Pool', value: formatCurrency(estimatedTaxPool), icon: PiggyBank, color: 'text-teal-600', bg: 'bg-teal-50' },
  ];
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-200">
      ...
      <div className="flex bg-white border border-gray-200 rounded-xl overflow-hidden">
        {(['monthly', 'quarterly'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`flex-1 py-2.5 text-sm font-semibold transition-colors capitalize flex items-center justify-center gap-2 ${
              activeTab === tab ? 'bg-[#050A30] text-white' : 'text-gray-500 hover:bg-gray-50'
            }`}>
            {tab === 'monthly' ? <Calculator size={14} /> : <BarChart3 size={14} />}
            {tab}
          </button>
        ))}
      </div>
      ...
      <div className="flex items-center gap-3 bg-white rounded-xl border border-gray-200 p-4 flex-wrap">
        ...
        {/* Month navigator — monthly tab only */}
        {activeTab === 'monthly' && (
        ...
        {/* Quarter select — only shown in quarterly tab */}
        {activeTab === 'quarterly' && (
      ...
      {activeTab === 'quarterly' && (
        <>
          {ql ? (
            <div className="flex justify-center py-12"><Loader2 className="animate-spin text-gray-300" size={24} /></div>
          ) : qRevenue === 0 && qExpenses === 0 && qVatIn === 0 ? (
            <div className="text-center py-10 text-gray-400">
              <BarChart3 size={32} className="mx-auto mb-2 opacity-30" />
              <p className="font-medium text-gray-500 text-sm">No data for Q{selectedQuarter} {selectedYear}</p>
              <p className="text-xs mt-1">Record sales or expenses in this quarter to see the breakdown</p>
            </div>
          ) : (
            <>
              {/* Section label */}
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-bold text-gray-800">Q{selectedQuarter} {selectedYear} — Quarterly Breakdown</p>
              </div>

              {/* Quarterly metric cards */}
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                {[
                  { label: 'Q Revenue',       value: formatCurrency(qRevenue),          icon: TrendingUp,    color: 'text-green-600',  bg: 'bg-green-50'         },
                  { label: 'Q Expenses',      value: formatCurrency(qExpenses),         icon: Receipt,       color: 'text-red-600',    bg: 'bg-red-50'           },
                  { label: 'Q Net Profit',    value: formatCurrency(qProfit),           icon: DollarSign,    color: qProfit >= 0 ? 'text-blue-600' : 'text-red-600', bg: qProfit >= 0 ? 'bg-blue-50' : 'bg-red-50' },
                  { label: 'Q VAT Collected', value: formatCurrency(qVatIn),            icon: ArrowUpRight,  color: 'text-purple-600', bg: 'bg-purple-50'        },
                  { label: 'Q VAT Paid',      value: formatCurrency(qVatOut),           icon: ArrowDownRight,color: 'text-orange-600', bg: 'bg-orange-50'        },
                  { label: 'Q Net VAT',       value: formatCurrency(Math.max(0,qNetVat)),icon: Calculator,   color: 'text-[#050A30]',  bg: 'bg-[#050A30]/5'      },
                  { label: 'Q Est. Tax (30%)',value: formatCurrency(qEstTax),           icon: PiggyBank,     color: 'text-teal-600',   bg: 'bg-teal-50'          },
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
                  ...
                  <p className="text-xl font-bold text-blue-800">{formatCurrency(qVatIn)}</p>
                  <p className="text-xs text-blue-500 mt-1">Q{selectedQuarter} total</p>
                </div>
                <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                  <div className="flex items-center gap-2 mb-1">
                    <ArrowDownRight size={14} className="text-red-500" />
                  ...
                  <p className="text-xl font-bold text-red-700">{formatCurrency(qVatOut)}</p>
                  <p className="text-xs text-red-400 mt-1">Q{selectedQuarter} total</p>
                </div>
              </div>

              {/* Net VAT payable callout */}
              {qNetVat > 0 && (
                <div className="flex items-start gap-3 px-4 py-3 bg-orange-50 border border-orange-200 rounded-xl">
                  <AlertTriangle size={16} className="text-orange-600 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-sm font-bold text-orange-700">Quarterly VAT Payable</p>
                    <p className="text-xs text-orange-600 mt-0.5">
                      Net VAT of {formatCurrency(qNetVat)} due for Q{selectedQuarter} {selectedYear}. Remit to FIRS before the quarter-end deadline.
                    </p>
                  </div>
                </div>
              )}
